...
export default function AdminAppointments() {
  ...
  const googleMeetQuery = useQuery({ queryKey: ["google-meet-control"], queryFn: async () => { const { data, error } = await supabase.functions.invoke("google-meet-control", { body: { action: "status" } }); if (error) throw error; return data?.enabled; } });
  ...
  const googleMeetMutation = useMutation({ mutationFn: async (enabled) => { const { data, error } = await supabase.functions.invoke("google-meet-control", { body: { enabled, requestId: crypto.randomUUID() } }); if (error) throw error; return data?.enabled; } });
  const closeCancellation = () => { setSelected(null); setCancellationReasonCode(""); setCancellationExplanation(""); };
