...
const content = {
  ...
  blog: { eyebrow: "Orion stories", title: "Experiences and reflections", text: "Read reflections from the Orion community. Patient-submitted experiences are reviewed before they are published.", details: ["“The booking process was easy to follow.” — Maria L.", "“I felt heard and supported.” — Jon R.", "Want to share your experience?"] },
};

export default function MarketingPage({ type }) {
  const page = content[type];
  return <section className="marketing-page"><p className="eyebrow">{page.eyebrow}</p><h1>{page.title}</h1><p className="marketing-lead">{page.text}</p><div className="marketing-details">{page.details.map((detail) => <div key={detail}>{detail}</div>)}</div>{type === "blog" ? <Link to="/submit-blog" className="action-button">Share your experience</Link> : <Link to="/patient-appointment" className="action-button">Book a Session</Link>}</section>;
}
