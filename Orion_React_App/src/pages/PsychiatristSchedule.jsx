...
export default function PsychiatristSchedule() {
  ...
  return <section className="scheduling-page">
    ...
    <section className="schedule-editor" aria-labelledby="recurring-schedule-title"><div className="booking-step__heading"><span className="booking-step__number">1</span><div><h2 id="recurring-schedule-title">Recurring weekday period</h2><p>Changes apply only to future unbooked availability.</p></div></div><form className="schedule-form" onSubmit={(event) => { event.preventDefault(); save("save-rule", { weekday: Number(rule.weekday), startsLocal: rule.startsLocal, endsLocal: rule.endsLocal }, "The recurring schedule period was saved."); }}><label>Weekday<select value={rule.weekday} onChange={(event) => setRule({ ...rule, weekday: event.target.value })}>{dayNames.map((day, index) => <option value={index + 1} key={day}>{day}</option>)}</select></label><label>Starts<input type="time" step="900" min="08:00" max="16:15" value={rule.startsLocal} onChange={(event) => setRule({ ...rule, startsLocal: event.target.value })} /></label><label>Ends<input type="time" step="900" min="08:45" max="17:00" value={rule.endsLocal} onChange={(event) => setRule({ ...rule, endsLocal: event.target.value })} /></label><Button type="submit" busy={mutation.isPending}>Save recurring period</Button></form>{rules.length > 0 && <div className="schedule-record-list"><h3>Current recurring periods</h3>{rules.map((item) => <div className="schedule-record" key={item.id}><span>{dayNames[item.weekday - 1]} · {item.starts_local.slice(0, 5)}–{item.ends_local.slice(0, 5)}</span><Button variant="quiet" onClick={() => deleteRule(item.id)}>Remove</Button></div>)}</div>}</section>
