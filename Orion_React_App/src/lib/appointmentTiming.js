...
export const EARLY_JOIN_MILLISECONDS = 15 * 60 * 1000;
...
export function isInMeetingWindow(startsAt, endsAt, now = new Date()) {
  const currentTime = new Date(now).getTime();
  return currentTime >= new Date(startsAt).getTime() - EARLY_JOIN_MILLISECONDS
