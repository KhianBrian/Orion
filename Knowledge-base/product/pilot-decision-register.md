...
**Amendment, 27 August 2026.** The owner review recorded below permits Daily, or public Jitsi as a fallback, for the demo only — in a clearly labelled internal fake-data mode using these synthetic accounts. It also introduced a secretary role and session notes. Whether the demo account set expands to include a secretary account, and whether session notes appear in the demo, are outstanding items below.
...
| 4 | Which patients are eligible, and what emergency/crisis path applies when Orion is not appropriate? | Adults only initially; no emergency care; provide approved local emergency/crisis and referral route. | Adopted as suggested. Adults only initially. Orion is not emergency care. An approved local emergency/crisis contact and referral route is displayed. | Answered | Sign-up and consultation |
...
| 8 | Which private video provider is approved after privacy, clinical, legal/DPO, security, and operations review? | Evaluate Daily first using its free tier; private rooms and short-lived server tokens only. | **Demo:** Daily preferred; public Jitsi permitted as a fallback for the demo only, in a clearly labelled internal fake-data mode with synthetic accounts. **Real launch:** provider decision deferred to a future owner decision. | Answered for demo; open for launch | Real video sessions |
...
- **Q1 — cap value and geography.** The cap's existence is decided; its value, mechanism (waitlist, approval queue, or hard limit), the approved geography, and the operational review cadence are not. *Blocks: Phase 3 registration gating, Phase 6 release scoping.*
...
- [Product scope](product-scope.md) — session notes and the secretary role added; notes carved out of the *Out of scope* clinical-records line; public-but-capped registration recorded.
...
- [Production service charter](production-service-charter.md) — service boundary amended for notes, demo mode amended for the labelled Jitsi fallback, pilot constraints amended for the capped public registration.
