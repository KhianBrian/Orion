...
| 8 | Which private video provider is approved after privacy, clinical, legal/DPO, security, and operations review? | Evaluate Daily first using its free tier; private rooms and short-lived server tokens only. | **Demo:** JaaS remains historical synthetic evidence. **Phase 18.5 synthetic slice:** Direct WebRTC + project-owned TURN is authorized for non-production implementation with dedicated signaling and disabled-by-default admission. **Real launch:** provider direction remains a separate owner/DPO/clinical/security/operations decision. | Synthetic implementation authorized; real-user provider pending | Real video sessions |
