...
| Reconnect and negotiation | ICE restart, gateway reconnect, mobile handoff, duplicate offer collision | Branch implementation verified locally: bounded reauthorization/reconnect, deterministic negotiation, expiry/revocation UI, queued ICE, and bounded ICE restart | Repeat against the configured current runtime; verify real mobile handoff, network interruption recovery, and no duplicate session creation |
...
| Operations | Health checks, alerts, dashboards, runbooks, on-call ownership | Signaling gateway now exposes `/healthz` and handles SIGTERM/SIGINT cleanly; alerting and ownership are not configured | Put the gateway behind the approved TLS/proxy host, configure monitoring and alert delivery, test stop authority, and record incident/recovery evidence |
