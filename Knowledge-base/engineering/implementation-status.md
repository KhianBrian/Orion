...
| D3 — Database-backed scheduling UI | Not started | The database can store the required core records, but no React page reads it yet. | Replace the duplicate mock booking routes with one role-correct, database-backed workflow. |
| D4 — Booking and cancellation boundary | Not started | Database constraints provide only the foundation; no booking or cancellation transaction function is deployed. | Implement and test the approved server-side transaction boundaries after confirming the applicable lifecycle decisions. |
...
Build D3's single database-backed scheduling workflow from the server-held availability data. The
legacy mock scheduling, appointment, and video pages must stay unavailable; do not add a secretary
