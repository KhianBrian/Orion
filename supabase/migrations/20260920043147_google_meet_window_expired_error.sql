-- Preserve a distinct server-side reason for an expired Google Meet window so
-- the UI can explain why a stale appointment page can no longer open Meet.
...
begin
  ...
  if now() >= appointment_row.ends_at then
    ...
    raise exception 'google_meet_window_expired' using errcode = '42501';
  end if;
